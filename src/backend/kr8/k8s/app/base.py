...
from src.backend.kr8.app.base import AppBase
from src.backend.kr8.app.context import ContainerContext
from src.backend.kr8.k8s.app.context import K8sBuildContext
...
if TYPE_CHECKING:
    from src.backend.kr8.k8s.resource.base import K8sResource
...
class K8sApp(AppBase):
    ...
    def get_cr_name(self) -> str:
        from src.backend.kr8.utils.defaults import get_default_cr_name

        return self.cr_name or get_default_cr_name(self.name)

    def get_crb_name(self) -> str:
        from src.backend.kr8.utils.defaults import get_default_crb_name

        return self.crb_name or get_default_crb_name(self.name)

    def get_configmap_name(self) -> str:
        from src.backend.kr8.utils.defaults import get_default_configmap_name

        return self.configmap_name or get_default_configmap_name(self.name)

    def get_secret_name(self) -> str:
        from src.backend.kr8.utils.defaults import get_default_secret_name

        return self.secret_name or get_default_secret_name(self.name)

    def get_container_name(self) -> str:
        from src.backend.kr8.utils.defaults import get_default_container_name

        return self.container_name or get_default_container_name(self.name)

    def get_deploy_name(self) -> str:
        from src.backend.kr8.utils.defaults import get_default_deploy_name

        return self.deploy_name or get_default_deploy_name(self.name)

    def get_pod_name(self) -> str:
        from src.backend.kr8.utils.defaults import get_default_pod_name

        return self.pod_name or get_default_pod_name(self.name)

    def get_service_name(self) -> str:
        from src.backend.kr8.utils.defaults import get_default_service_name
            ...
            if self.enable_https:
                ...
                if self.acm_certificate_summary_file is not None and isinstance(
                    ...
                    if self.acm_certificate_summary_file.exists() and self.acm_certificate_summary_file.is_file():
                        from src.backend.kr8.aws.resource.acm.certificate import CertificateSummary
    ...
    def get_ingress_name(self) -> str:
        from src.backend.kr8.utils.defaults import get_default_ingress_name
    ...
    def get_cr_policy_rules(self) -> List[Any]:
        from src.backend.kr8.k8s.create.rbac_authorization_k8s_io.v1.cluster_role import (
            PolicyRule,
        )
    ...
    def get_container_env(self, container_context: ContainerContext) -> Dict[str, str]:
        from src.backend.kr8.constants import (
            PHI_RUNTIME_ENV_VAR,
            PYTHONPATH_ENV_VAR,
    ...
    def build_resources(self, build_context: K8sBuildContext) -> List["K8sResource"]:
        ...
        from src.backend.kr8.k8s.create.base import CreateK8sResource
        ...
        from src.backend.kr8.k8s.create.core.v1.volume import (
            CreateVolume,
            HostPathVolumeSource,
            AwsElasticBlockStoreVolumeSource,
            VolumeType,
        )
        from src.backend.kr8.k8s.create.networking_k8s_io.v1.ingress import CreateIngress
        from src.backend.kr8.k8s.create.rbac_authorization_k8s_io.v1.cluste_role_binding import CreateClusterRoleBinding
        from src.backend.kr8.k8s.create.rbac_authorization_k8s_io.v1.cluster_role import CreateClusterRole
        from src.backend.kr8.k8s.resource.base import K8sResource
        from src.backend.kr8.k8s.resource.yaml import YamlResource
        from src.backend.kr8.utils.defaults import get_default_volume_name, get_default_sa_name
