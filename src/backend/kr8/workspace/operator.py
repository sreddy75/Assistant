...
from src.backend.kr8.api.workspace import log_workspace_event
from src.backend.kr8.api.schemas.workspace import (
    WorkspaceSchema,
    WorkspaceCreate,
    WorkspaceUpdate,
    WorkspaceEvent,
...
from src.backend.kr8.cli.config import PhiCliConfig
from src.backend.kr8.cli.console import (
    console,
    print_heading,
    print_info,
    print_subheading,
...
from src.backend.kr8.infra.type import InfraType
from src.backend.kr8.infra.resources import InfraResources
from src.backend.kr8.workspace.config import WorkspaceConfig
from src.backend.kr8.workspace.enums import WorkspaceStarterTemplate
...
def create_workspace(name: Optional[str] = None, template: Optional[str] = None, url: Optional[str] = None) -> bool:
    ...
    from src.backend.kr8.cli.operator import initialize_phi
    from src.backend.kr8.utils.common import str_to_int
    from src.backend.kr8.utils.filesystem import rmdir_recursive
    from src.backend.kr8.workspace.helpers import get_workspace_dir_path
    from src.backend.kr8.utils.git import GitCloneProgress
    ...
    if not phi_config:
        ...
        if not init_success:
            from src.backend.kr8.cli.console import log_phi_init_failed_msg
...
def setup_workspace(ws_root_path: Path) -> bool:
    ...
    """
    from src.backend.kr8.cli.operator import initialize_phi
    from src.backend.kr8.utils.git import get_remote_origin_for_dir
    from src.backend.kr8.workspace.helpers import get_workspace_dir_path
    ...
    if phi_config.user is None:
        from src.backend.kr8.api.user import create_anon_user
    ...
    if phi_config.user is not None:
        ...
        if ws_schema is None or ws_schema.id_workspace is None:
            from src.backend.kr8.api.workspace import create_workspace_for_user
            from src.backend.kr8.workspace.helpers import generate_workspace_name
        ...
        ######################################################
            ...
            from src.backend.kr8.api.workspace import update_primary_workspace_for_user
        ...
        if ws_schema is not None and ws_schema.git_url != git_remote_origin_url:
            from src.backend.kr8.api.workspace import update_workspace_for_user
